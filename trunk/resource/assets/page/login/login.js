...
var LoginStep1 = React.createClass({
	...
	subClick:function(){
		...
		var _this = this;
		$.sendData({
	        url:types.API+"/authorize/MainServlet?method=quickLogin&req_fmt_type=jsonp&password=&auth_type=1&cookie_scope=11102&isQuick=1&isPicCode="+(isPicCode-1)+picCodeTxt+"&sms_code="+vcode+"&smsCheckCode="+vcode+"&buyer_name="+_this.props.mobile+"&uniqueKey="+uniqueKey, 
	        success: function(data){
            	if(data.Result.Header.resultID == 0){
					_this.setState({
						errorMsg:""
